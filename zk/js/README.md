...
This directory contains the retained WebSnark prover and SnarkJS verifier.
The Python backend is the default; set `ZK_BACKEND = "javascript"` in `config.py` to use this implementation.

Node.js is not bundled. The `node` command must be available on `PATH`.

The backend uses the same Tornado Cash v2.1 circuit and keys as the Python backend.
It returns `pi_a`, `pi_b`, `pi_c`, six `publicSignals`, and a 256-byte Solidity proof.
Proofs generated by either backend verify in both.
...
__Update__  
With help from Codex, `circuit.json` can now be translated ahead of time into Python.  
The generated module is stored in `zk/groth16/_witness_program.py` and produced by `zk/groth16/generate_witness_program.py`.
